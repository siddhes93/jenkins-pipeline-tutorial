...
pipeline {
    ...
    environment {
	    region = "us-east-1"
             ...
             task_def_arn = "arn:aws:ecs:us-east-1:466037112537:task-definition/first-run-task-definition:6"
            cluster = "sample-app"
	...
	stage('Build') {
     	...
     	stage('Deploy') {
             ...
             sh "aws ecs update-service --cluster ${cluster} --service sample-java --task-definition ${task_def_arn} --region ${region}"
                 }
            }
